...
async def browser_voice_test(websocket: WebSocket):
    ...
    async def on_call_end():
        ...
        # Send recording to browser FIRST (before slow pipeline.stop)
        wav_data = None
        try:
        ...
        # Upload to storage after pipeline cleanup (non-blocking for browser)
...
# ─── V1 API — AI SDR Integration ───
# These endpoints are called by the AI SDR platform's cogniflow.ts integration
# to initiate outbound sales calls and check call status.
...
async def v1_outbound_call(request: Request, auth: AuthContext = Depends(get_auth_context)):
    ...
    try:
        ...
        if result.call_sid:
            ...
            # Store SDR context for when the call connects
            call_metadata = {
                "source": "ai_sdr",
                "sdr_context": context,
