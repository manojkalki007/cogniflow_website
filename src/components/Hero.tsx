...
import { FadeUp, FadeIn, ScaleUp } from "./animations";
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-4 sm:p-5 bg-gray-50/50 min-h-[280px] sm:min-h-[340px]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            {[
              { label: "Total Calls", value: "12,847", change: "+23%" },
              { label: "Active Agents", value: "24", change: "+3" },
              { label: "Avg Duration", value: "3:42", change: "-8%" },
              { label: "Conversion", value: "34.2%", change: "+5.1%" },
            ].map((stat) => (
              <div key={stat.label} className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm">
                ...
                <p className="text-[10px] font-medium text-emerald-500 mt-0.5">{stat.change} vs last month</p>
              </div>
            ))}
          </div>
            ...
            <div className="flex items-end gap-[6px] h-24">
              {barHeights.map((h, i) => (
                <div key={i} className="flex-1 flex flex-col gap-[2px] items-stretch">
                  <div className="rounded-t-sm bg-[var(--color-brand)] opacity-80" style={{ height: `${h}%` }} />
                  <div className="rounded-b-sm bg-cyan-300 opacity-60" style={{ height: `${Math.max(10, h * 0.4)}%` }} />
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto text-center">
        ...
        <FadeUp delay={0.3}>
          <div className="flex items-center justify-center gap-4 mt-8 flex-wrap">
            ...
            <a href={CAL_LINK} target="_blank" rel="noopener noreferrer" className="btn-ghost text-base !py-3.5 !px-7">
